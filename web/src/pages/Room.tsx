...
import { Button, NavBar, Dialog, Form, Input, Stepper, Toast } from 'antd-mobile'
...
const Room = () => {
    ...
    useEffect(() => {
        ...
        return () => {
            // 组件卸载时，如果有当前房间且状态为等待中，则离开房间
            if (currentRoom && currentRoom.status === GameStatus.WAITING) {
                leaveRoom(currentRoom.id)
            }
        }
    }, [roomId, currentRoom, joinRoom, leaveRoom, setupRoomEvents, navigate])

    // 监听房间状态变化，如果游戏开始则跳转到游戏页面
    useEffect(() => {
        if (currentRoom && (currentRoom.status === GameStatus.PLAYING || currentRoom.status === GameStatus.VOTING)) {
    ...
    useEffect(() => {
        if (currentRoom?.config) {
            ...
            // 如果是房主且没有配置，则设置默认值
            const playerCount = Object.keys(currentRoom.players).length - 1 // 减去房主
            setConfigForm({
                goodWord: '',
                badWord: '',
                goodCount: Math.max(Math.floor(playerCount * 0.7), 1),
                badCount: Math.max(Math.floor(playerCount * 0.2), 1),
                blankCount: Math.max(playerCount - Math.floor(playerCount * 0.7) - Math.floor(playerCount * 0.2), 0)
            })
        }
    }, [currentRoom, isHost])
    ...
    const handleUpdateConfig = async () => {
        ...
        const playerCount = Object.keys(currentRoom?.players || {}).length - 1 // 减去房主
        if (configForm.goodCount + configForm.badCount + configForm.blankCount !== playerCount) {
            Toast.show({
                content: `角色数量(${configForm.goodCount + configForm.badCount + configForm.blankCount})与玩家数量(${playerCount})不一致`,
                position: 'bottom',
            })
            return
        }
    ...
    return (
        <div className="page-container">
            <NavBar
                ...
                right={isHost ? null : <Button size="small" onClick={handleLeaveRoom}>退出</Button>}
            >
                房间号: {roomId}
            </NavBar>
            ...
            <div className="card">
                ...
                <div className="player-list">
                    {Object.values(currentRoom.players).map(player => (
                        ...
                        >
                            {player.username}
                            {player.isHost && <div style={{ fontSize: '12px', color: 'var(--primary-color)' }}>(主持人)</div>}
                        </div>
                    ))}
                </div>
            </div>
                    ...
                    <Form layout="horizontal" style={{ marginTop: '16px' }}>
                        <Form.Item label="好人词">
                            <Input
                                ...
                                onChange={val => setConfigForm(prev => ({ ...prev, goodWord: val }))}
                            />
                        </Form.Item>
                        <Form.Item label="坏人词">
                            <Input
                                ...
                                onChange={val => setConfigForm(prev => ({ ...prev, badWord: val }))}
                            />
                        </Form.Item>
                        <Form.Item label="好人数量">
                            <Stepper
                                ...
                                onChange={val => setConfigForm(prev => ({ ...prev, goodCount: val }))}
                            />
                        </Form.Item>
                        <Form.Item label="坏人数量">
                            <Stepper
                                ...
                                onChange={val => setConfigForm(prev => ({ ...prev, badCount: val }))}
                            />
                        </Form.Item>
                        <Form.Item label="白板数量">
                            <Stepper
                                ...
                                onChange={val => setConfigForm(prev => ({ ...prev, blankCount: val }))}
                            />
                        </Form.Item>
                    </Form>
                ...
                <div className="card">
                    ...
                    <div className="button-group" style={{ justifyContent: 'space-between' }}>
                        <Button
                            ...
                            loading={loading}
                            disabled={loading || !currentRoom.config?.goodWord || !currentRoom.config?.badWord}
                        >
                            随机分配角色
                        </Button>
                        <Button
                            ...
                            disabled={
                                ...
                                Object.values(currentRoom.players).some(p => p.role === undefined && !p.isHost)
                            }
                        >
                            开始游戏
                        </Button>
