...
const Home = () => {
  ...
  const { rooms, fetchRooms, createRoom, joinRoom, setupRoomEvents } =
    useRoomStore();
  ...
  const handleJoinRoom = async () => {
    try {
      ...
      Toast.show({
        content:
          error instanceof Error ? error.message : "加入房间失败，请重试",
        position: "bottom",
      });
    } finally {
      setLoading(false);
    ...
    <div className="page-container">
      ...
      <div className="card">
        ...
        {activeRooms.length > 0 ? (
          ...
          <Empty style={{ padding: "32px 0" }} description="暂无可用房间" />
        )}
      </div>
    </div>
  );
