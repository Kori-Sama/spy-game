...
import { Button, NavBar, Dialog, Toast, Form, Input, Grid, Space, Radio } from 'antd-mobile'
...
const Game = () => {
    ...
    // 游戏阶段
    const isWaiting = currentRoom?.status === GameStatus.WAITING
    ...
    // 玩家需要知道的词
    const playerWord = playerRole === PlayerRole.GOOD
        ? currentRoom?.config?.goodWord
        : (playerRole === PlayerRole.BAD ? currentRoom?.config?.badWord : '');
    ...
    const renderWinnerInfo = () => {
        ...
        return (
            <div className="winner-display">{message}</div>
        )
    }
    ...
    return (
        <div className="page-container">
            <NavBar
                ...
                backArrow={isEnded}
                right={isEnded ? null : <Button size="small" onClick={handleBackToLobby}>退出</Button>}
            >
                ...
                <div className="card">
                    ...
                    <div className="game-info">
                        {playerRole === PlayerRole.GOOD && (
                            <>
                                <p>你是<strong>好人</strong>，你的词语是：</p>
                                <div className="word-display">{playerWord}</div>
                                <p>尝试找出坏人和白板，不要让他们猜出你的词语！</p>
                            </>
                        )}

                        {playerRole === PlayerRole.BAD && (
                            <>
                                <p>你是<strong>坏人</strong>，你的词语是：</p>
                                <div className="word-display">{playerWord}</div>
                                <p>尝试伪装成好人，但不要暴露你的词语！你需要猜出好人的词语。</p>
                            </>
                        )}

                        {playerRole === PlayerRole.BLANK && (
                            <>
                                <p>你是<strong>白板</strong>，你没有词语。</p>
            ...
            <div className="card">
                ...
                <div className="player-list">
                    {Object.values(currentRoom.players).map(player => {
                        ...
                        // 是否已被当前用户投票
                        const isVoted = currentPlayer?.voted === player.id
                        ...
                        return (
                            ...
                            >
                                ...
                                {(!isHost && user?.id === player.id && playerRole) && renderRoleBadge(playerRole)}
                                    ...
                                    {livingPlayers
                                        ...
                                        .map(player => (
                                            <Radio key={player.id} value={player.id}>
                                                {player.username} {currentPlayer?.voted === player.id && '(已投票)'}
                                            </Radio>
                                        ))
                                    }
                                </Space>
            ...
            {isPlaying && isAlive && !isHost && (playerRole === PlayerRole.BAD || playerRole === PlayerRole.BLANK) && (
                <div className="card">
                    <h2>猜词</h2>
            ...
            {isEnded && (
                <div className="card">
                    ...
                    <div className="button-group" style={{ justifyContent: 'center', marginTop: '16px' }}>
                        <Button color="primary" onClick={handleBackToLobby}>返回大厅</Button>
                    </div>
                </div>
            )}
        </div>
