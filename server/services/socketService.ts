  ...
  io.on("connection", (socket: TypedSocket) => {
    ...
    socket.on("update_room_config", async (data) => {
      try {
        ...
        const updatedRoom = await roomService.updateRoomConfig(
          data.roomId,
          data.config
        );
    ...
    socket.on("assign_roles", async (data) => {
      try {
        ...
        const updatedRoom = await roomService.assignRoles(
          data.roomId,
          data.assignments
        );
    ...
    socket.on("start_vote", async (data) => {
      try {
        ...
        socket.emit("error", {
          message: error instanceof Error ? error.message : "开始投票失败",
        });
      }
    });
    ...
    socket.on("end_vote", async (data) => {
      try {
        ...
        socket.emit("error", {
          message: error instanceof Error ? error.message : "结束投票失败",
        });
      }
    });
    ...
    socket.on("vote", async (data) => {
      try {
        ...
        const result = await roomService.vote(
          data.roomId,
          socket.data.user.id,
          data.targetId
        );
    ...
    socket.on("guess_word", async (data) => {
      try {
        ...
        const result = await roomService.guessWord(
          data.roomId,
          socket.data.user.id,
          data.word
        );
        ...
        socket.emit("error", {
          message: error instanceof Error ? error.message : "猜词失败",
        });
      }
    });
    ...
    socket.on("remove_player", async (data) => {
      try {
        ...
        if (room.hostId !== socket.data.user.id) {
          socket.emit("error", { message: "只有主持人可以移除玩家" });
          return;
        }

        const updatedRoom = await roomService.removePlayer(
          data.roomId,
          data.playerId
        );
        ...
        console.error("移除玩家错误:", error);
        socket.emit("error", {
          message: error instanceof Error ? error.message : "移除玩家失败",
        });
      }
    });
...
// 广播房间列表更新
async function broadcastRoomsUpdate(
  io: Server<ClientToServerEvents, ServerToClientEvents>
) {
  try {
